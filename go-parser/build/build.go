...
import (
	...
	"github.com/TobiasYin/extgo/go-parser/build/constraint"
	"github.com/TobiasYin/extgo/go-parser/doc"
...
// in Go modules. In the long term we want tools to use github.com/TobiasYin/extgo/go-parser/packages (currently golang.org/x/tools/github.com/TobiasYin/extgo/go-parser/packages),
...
func (ctxt *Context) importGo(p *Package, path, srcDir string, mode ImportMode) error {
	...
	if srcDir != "" {
		...
		if filepath.IsAbs(srcDir) {
			...
			return fmt.Errorf("github.com/TobiasYin/extgo/go-parser/build: Dir is non-empty, so relative srcDir is not allowed: %v", srcDir)
		} else {
	...
	if err := cmd.Run(); err != nil {
		return fmt.Errorf("github.com/TobiasYin/extgo/go-parser/build: go list %s: %v\n%s\n", path, err, stderr.String())
	}

	f := strings.SplitN(stdout.String(), "\n", 5)
	if len(f) != 5 {
		return fmt.Errorf("github.com/TobiasYin/extgo/go-parser/build: importGo %s: unexpected output:\n%s\n", path, stdout.String())
	}
	dir := f[0]
