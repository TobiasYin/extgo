...
import (
	...
	"github.com/TobiasYin/extgo/go-parser/scanner"
...
func assert(cond bool, msg string) {
	if !cond {
		panic("github.com/TobiasYin/extgo/go-parser/parser internal error: " + msg)
	}
}
...
func (p *parser) parsePrimaryExpr() (x ast.Expr) {
	...
	for {
		...
		// Tobias 扩展
		case token.QUES:
			x = &ast.QuesExpr{Ques: p.pos, X: p.checkExpr(x)} // TODO check X must be call expr
			p.next()
		case token.PERIOD:
			p.next()
			switch p.tok {
...
func isValidImport(lit string) bool {
	const illegalChars = `!"#$%&'()*,:;<=>?[\]^{|}` + "`\uFFFD"
	s, _ := strconv.Unquote(lit) // github.com/TobiasYin/extgo/go-parser/scanner returns a legal string literal
	for _, r := range s {
